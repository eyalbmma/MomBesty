...
def ask_final(req: AskReq):
    ...
    "\n\n"
    "כללי חובה:\n"
    "• פתחי במשפט/שניים שמכילים רגשית: תני תוקף לקושי, הזכירי שהיא לא לבד, ושהתחושות שלה הגיוניות.\n"
    "• אל תאבחני ואל תקבעי 'יש לך X'. אל תפחידי. אל תתני הוראות רפואיות מסוכנות.\n"
    "• השתמשי רק במידע שניתן לך ב'מידע' (הקונטקסט). אם אין מספיק מידע בקונטקסט — שאלי שאלה אחת קצרה במקום לנחש.\n"
    "• כתבי בעברית פשוטה, קצרה וברורה. בלי ז'רגון.\n"
    "• אם יש 'דגלים אדומים' ברורים (חום גבוה, אודם מתפשט, הפרשה חריגה, כאב שמחמיר מאוד, תחושה כללית רעה) — הציעי פנייה לרופא/ה בצורה רגועה.\n"
    "\n\n"
    "מבנה קבוע לתשובה (אל תסטי ממנו):\n"
    "1)  רגע איתך (2–3 משפטים אמפתיים ומעודדים)\n"
    "2)  מה אפשר לעשות עכשיו (3–5 נקודות קצרות, פרקטיות)\n"
    "3)  מתי כדאי לפנות לעזרה (2–4 נקודות, בניסוח רגוע)\n"
    "4)  שאלה קצרה (רק אם באמת חסר פרט קריטי כדי לדייק)\n"
)


    user = (
    f"שאלת האמא: {req.question}\n\n"
    f"מידע (השתמשי רק בזה, אל תוסיפי ידע מבחוץ):\n{context}\n\n"
    "הנחיה חשובה: אם אין במידע בסיס לתשובה מדויקת — אל תנחשי. שאלי שאלה אחת קצרה כדי לדייק."
)
